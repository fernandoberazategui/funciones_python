...
for t in temp_dataloger:
    if (temperatura_max is None) or (t >= temperatura_max):
        temperatura_max = t
    if (temperatura_min is None) or (t <= temperatura_min):
        temperatura_min = t
temperatura_len += 1
temperatura_sumatoria += t
temperatura_promedio= temperatura_sumatoria/temperatura_len
print(temp_dataloger)
print("Según las muestras se obtuvieron estos datos:\n"
"Máxima temperatura:", temperatura_max,"\n"
"Mínima temperatura:", temperatura_min,"\n"
...
temp_max_py = max(temp_dataloger)
Temp_min_py = min(temp_dataloger)

print("temp maxima py:{} y temp minima py {} coinciden".format(temp_max_py,Temp_min_py))
